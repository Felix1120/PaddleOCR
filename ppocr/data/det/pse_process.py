...
class PSEProcessTrain(object):
    """
    PSE pre-process for Train mode
    """
    ...
    def __call__(self, label_infor):
        ...
        for i in range(1, self.n + 1):
            shrink_ratio = 1 - (1 - self.m) * (self.n - i) / (self.n - 1)
            tmp_data = MakeShrinkMap(copy_data, shrink_ratio)
            shrink_maps.append(tmp_data['shrink_map'])
            shrink_masks.append(tmp_data['shrink_mask'])
        data['shrink_maps'] = np.array(shrink_maps, dtype=np.float32)
        data['masks'] = shrink_masks[-1]
        data = self.NormalizeImage(data)
        data = self.FilterKeys(data)
        return data['image'], data['shrink_maps'], data['masks']


class PSEProcessTest(object):
    """
    PSE pre-process for Test mode
    """
